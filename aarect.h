...
public:
    ...
    double d;
    rect_t()=default;
    rect_t(vec3_t normal,point3_t a,point3_t b,std::shared_ptr<material_t> mat):n(normal),min(a),max(b),mat_ptr(mat)
    {
        //min=vec3_t(fmin(a.x,b.x),fmin(a.y,b.y),fmin(a.z,b.z));
        //max=vec3_t(fmax(a.x,b.x),fmax(a.y,b.y),fmax(a.z,b.z));
        d=-dot(n,min);
    }
    virtual std::pair<bool, hit_record_t> hit(const ray_t &r, double t_min, double t_max) const
    {
        ...
        rec.u=(p.x-min.x)/(max.x-min.x);
        rec.v=(p.y-min.y)/(max.y-min.y);
        //fprintf(stderr,"%f %f\n",rec.u,rec.v);
        return {true,rec};
    }
